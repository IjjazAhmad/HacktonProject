...
export default function AddCourse() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault()
    let { courseName, code} = state

    if (courseName.length<3) { return   window.notify("Name manimum 3 char", "error") }
    ...
    try {
      ...
      window.notify("Add Course Successfully", "succes")
      setProgress(false)
    } catch (e) {
      window.notify("Coure not added Error!", "error");
    }
  }
  return (
    <>
